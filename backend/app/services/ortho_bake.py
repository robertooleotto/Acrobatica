...
def seal_texture_edges(image: np.ndarray) -> np.ndarray:
    """Estende il colore nei texel trasparenti e rende opaca la texture finale.

    Ogni PNG appartiene a un solo piano, quindi i pixel esterni al relativo
    poligono non vengono mai disegnati. Riempirli evita che il filtro bilineare
    campioni alfa zero proprio lungo gli spigoli condivisi fra due piani.
    """
...
def bake_planes(mesh_path: str, poses: dict, photos_dir: str, planes_doc: dict,
    ...
    for i, plane in enumerate(planes, 1):
        ...
        texture = seal_texture_edges(cv2.cvtColor(img, cv2.COLOR_RGBA2BGRA))
        cv2.imwrite(os.path.join(out_dir, fname), texture)
